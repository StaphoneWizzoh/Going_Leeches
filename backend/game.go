...
func (game *ChessGame) Join(ws *websocket.Conn) error{
	// One cannot join the same game twice
	...
	go fowardFromWebsocketToChannel(game.whiteWebsocket, whiteChannel)
	go fowardFromWebsocketToChannel(game.blackWebsocket, blackChannel)
...
func fowardFromWebsocketToChannel(ws *websocket.Conn, ch chan<-Message){
	defer ws.Close()
	for{
		message := Message{}
